...
im = transforms.Compose([
    transforms.Resize(256),  # 现在Scale已经被resize给替代
...
str_label_orig = labels[np.int_(label_orig)].split(',')[0]  # 原本的int函数已经被弃用，改为np.int_
...
plt.figure()
plt.imshow(tf(pert_image.cpu()[0]))    # 原本图像是500 * 600的，这里的图像会比较糊
